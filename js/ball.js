...
SB.Ball.prototype.draw = function(ctx) {
    ...
    var lowSpeedFade = Math.min(speedFrac * 5, 1);
    var trailAlphaMult = (1 + speedFrac * 1.5 + comboBoost * 1.0 + tb * 1.5) * (0.3 + lowSpeedFade * 0.7);
    ...
    var cw = SB.canvasWidth || 400;
    var shadowOffX = (this.x - cw / 2) / cw * 6 * (1 - heightFrac);
    ctx.save();
    ctx.fillStyle = 'rgba(0, 0, 0, ' + shadowAlpha.toFixed(2) + ')';
    ctx.translate(this.x + shadowOffX, this.y + this.radius + 6 + (1 - heightFrac) * 4);
